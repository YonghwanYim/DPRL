'''
I had to switch to the higher version, PyQt6, because using PyQt5 was difficult on the Mac M1 operating system.
Some codes based on the PyQt5 library do not work in PyQt6, so I had to modify the code.
The modified code is as follows :  66~68, 159~160, 304~305
'''
import sys
import numpy as np
...
# from DPAgentOnGW_Sol import DPAgentOnGW
from DPAgentOnGW import DPAgentOnG
...
class Main_GUI(QMainWindow):
    ...
    def Combobox_activated(self):
        ...
        if selected < 1:
            ...
            # self.PaintArea.setFrameStyle(QFrame.Panel)  # AttributeError : type object 'QFrame' has no attribute 'Panel'
            # In PyQt6, the frame style is set using setFrameShape() instead of the setFrameStyle() method.
            self.PaintArea.setFrameShape(QFrame.Shape.StyledPanel)
            self.area.vision_on()
            self.make_environment()
...
class DrawGraphs(QWidget):
    def __init__(self, main_gui):
        ...
        #self.setPalette(QPalette(Qt.dark))  # error : type object 'Qt' has no attribute 'dark'
        self.setPalette(QPalette(2))
            ...
            for x in range(self.nc):
                for y in range(self.nr):
                    ...
                    if self.plot_type == 0:  # Plot Q Value
                        ...
                        if VfromQ:
                            v = round(np.max(self.QValue[y, x, :]), 1)
                            color = np.max(C[y, x, :])
                        else:
                            v = round(self.VValue[y, x], 1)
                            color = C[y, x]
...
if __name__ == '__main__':
    ...
    # sys.exit(app.exec_())  # AttributeError 'Qapplication' object has no attribute 'exec_'
    sys.exit(app.exec()) # PyQt6 code.
